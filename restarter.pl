...
unless (-f PATH.'epoch') {
    print STDERR "Can't find the servers binary, Make sure you have the correct path in the restarter.pl !\n";
    exit;
}
...
print STDERR "Restarting Dayz Epoch server...\n";
chdir (PATH);

my $cmd = '/usr/bin/screen -h 20000 -fa -d -m -S epochServer '.PATH.'epoch.sh';
my $res = `$cmd`;
print STDERR $res,"\n" if $res;
exit;
